...
DAMAGE_WORDS = [
    ...
    "deformation", "broken", "leak", "leaking"
]
...
def add_text_features(out, df):
    ...
    for word in DAMAGE_WORDS:
        out["TEXT_HAS_" + word.upper().replace(" ", "_")] = text.str.contains(word, regex=False).astype(int)
    return out
...
def build_model():
    return HistGradientBoostingClassifier(
        max_iter=600, learning_rate=0.03, max_leaf_nodes=127, l2_regularization=0.1,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", type=Path, default=Path("more_remarks_words_submission.csv"))
