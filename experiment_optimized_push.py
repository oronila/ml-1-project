...
def make_features(df):
    ...
    out_dict["MONTH_SIN"] = np.sin(2 * np.pi * out_dict["MONTH"] / 12.0)
    out_dict["MONTH_COS"] = np.cos(2 * np.pi * out_dict["MONTH"] / 12.0)
    ...
    for col in ["HEIGHT", "SPEED", "DISTANCE"]:
        if col in out_dict:
            out_dict["LOG_" + col] = np.log1p(np.maximum(0, out_dict[col].fillna(0)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", type=Path, default=Path("optimized_push_submission.csv"))
