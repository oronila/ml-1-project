...
def freq_encode(train_X, val_X, test_X):
    for col in HIGH_CARD_COLS:
        if col in train_X.columns:
            ...
            # Rank encoding
            ranks = {val: i for i, val in enumerate(train_X[col].value_counts().index)}
            train_X["RANK_" + col] = train_X[col].map(ranks).fillna(len(ranks))
            val_X["RANK_" + col] = val_X[col].map(ranks).fillna(len(ranks))
            test_X["RANK_" + col] = test_X[col].map(ranks).fillna(len(ranks))
...
def build_model():
    return HistGradientBoostingClassifier(
        max_iter=800, learning_rate=0.04, max_leaf_nodes=63, l2_regularization=0.05,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", type=Path, default=Path("simple_strong_no_text_submission.csv"))
