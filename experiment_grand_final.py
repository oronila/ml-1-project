...
DROP_COLS = [ID_COL, TARGET, "LOCATION", "BIRD_BAND_NUMBER", "REG", "FLT", "AIRCRAFT", "LUPDATE", "COMMENTS"]
...
def make_features(df):
    ...
    out_dict["YEAR"] = dates.dt.year
    out_dict["MONTH"] = dates.dt.month
    out_dict["DAY_OF_YEAR"] = dates.dt.dayofyear
    out_dict["MONTH_SIN"] = np.sin(2 * np.pi * out["MONTH"] / 12.0) if False else np.sin(2 * np.pi * dates.dt.month / 12.0)
    # Wait, the previous logic was slightly flawed in make_features (using 'out' before defined).
    ...
    for col in ["HEIGHT", "SPEED", "DISTANCE"]:
        if col in out_dict:
            out_dict["LOG_" + col] = np.log1p(np.maximum(0, pd.to_numeric(out_dict[col], errors='coerce').fillna(0)))
    ...
    if "REMARKS" in df.columns:
        text = text + " " + df["REMARKS"].fillna("").astype(str).str.lower()
    ...
    # Fix YEAR, MONTH, DAY_OF_YEAR
    out["YEAR"] = dates.dt.year.fillna(2015)
    out["MONTH"] = dates.dt.month.fillna(6)
    out["DAY_OF_YEAR"] = dates.dt.dayofyear.fillna(180)
...
def prepare_train_val_test(train_df, val_df, test_df):
    ...
    ohe_cols = [c for c in CATEGORY_COLS if c in combined.columns]
    combined = pd.get_dummies(combined, columns=ohe_cols, dtype=float)
...
def build_model():
    return HistGradientBoostingClassifier(
        max_iter=900, learning_rate=0.02, max_leaf_nodes=127, l2_regularization=0.25,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", type=Path, default=Path("grand_final_submission.csv"))
