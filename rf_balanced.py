"""Random Forest with class_weight=balanced + target-rate threshold."""
...
def main():
    ...
    ap.add_argument("--out", default="rf_target_rate_submission.csv")
    ...
    # Use dense encoding so RF doesn't get categorical codes interpreted as ordinals
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_train, y), 1):
        ...
        rf.fit(X_train.iloc[tr_idx], y.iloc[tr_idx])
        oof[va_idx] = rf.predict_proba(X_train.iloc[va_idx])[:, 1]
